...
// Initialize floating social buttons
function initFloatingSocialButtons() {
    ...
    socialButtons.forEach(button => {
        ...
        button.addEventListener('click', function() {
            const platform = this.classList.contains('instagram-btn') ? 'Instagram' : 'WhatsApp';
...
function showNotification(message, type = 'info') {
    ...
    notification.style.cssText = `
        ...
        top: 20px;
        right: 20px;
        background: ${type === 'success' ? '#25D366' : type === 'error' ? '#DC3545' : '#FF6B35'};
        color: white;
        padding: 16px 20px;
        border-radius: 8px;
...
style.textContent = `
    ...
    /* Floating social buttons responsive adjustments */
...
// Initialize intersection observer for social buttons visibility
function initSocialButtonsVisibility() {
    const socialButtons = document.querySelector('.floating-social');
    ...
    if (socialButtons) {
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    socialButtons.style.transform = 'translateY(0)';
                } else {
                    socialButtons.style.opacity = '0.8';
                }
            });
        }, {
            threshold: 0.1
        ...
        // Observe the hero section to show/hide social buttons
...
// Initialize social buttons visibility on load
setTimeout(() => {
    initSocialButtonsVisibility();
}, 1000);